...
//      });
    ...
    //     phpinfo();
    // });
    Route::get('/manages/index', function () {
    ...
    // });
     ...
     Route::get('/api/wallet/transhistory', 'Api\WalletController@transhistory');
      
       Route::get('/api/test/oList', 'Api\UserController@OrderList');
    
     Route::get('/api/test/bList', 'Api\WalletController@BalanceList');
    
     Route::get('/api/test/walletList', 'Api\WalletController@walletList');
    
     Route::get('/api/news/listtest', 'Api\NewsController@getArticle');
    
    Route::get('/api/wallet/earnedhistory', 'Api\WalletController@earnedhistory');
    //新资产
    Route::get('/api/wallet/newlist', 'Api\WalletController@New_walletList');
    
    Route::get('/api/user/score', 'Api\UserController@getscore');  
     // 会员等级奖励列表
    Route::post('/api/user/level', 'Api\UserController@getlevel');  
    
    Route::post('/api/user/getusdt', 'Api\UserController@getamount');  
      //量化理财
      
    Route::get('/api/defi/defiList', 'Api\DefiController@defiList');  
    
    Route::post('/api/defi/buy', 'Api\DefiController@Buy');  
     
    Route::get('/api/defi/orderList', 'Api\DefiController@OrderList');  
      
    Route::get('/api/defi/Detail', 'Api\DefiController@Detail');   
    Route::get('/api/dual/testdetail', 'Api\DualController@getDetail');
    Route::get('/api/dual/index', 'Api\DualController@index');
    Route::get('/api/dual/dual_list', 'Api\DualController@dual_list');
    Route::get('/api/dual/dual_detail', 'Api\DualController@order_detail');
     Route::get('/api/dual/buyDualtest', 'Api\DualController@buyDual');
     
     //AI量化理财 
    Route::get('/api/ai/Detail', 'Api\AiController@getDetail');   
    Route::get('/api/ai/testdetail', 'Api\AiController@getDetail');
    Route::get('/api/ai/index', 'Api\AiController@index');
    Route::get('/api/ai/ai_list', 'Api\AiController@ai_list');
    Route::get('/api/ai/ai_detail', 'Api\AiController@order_detail');
     Route::get('/api/ai/buyAitest', 'Api\AiController@buyAi');
    //借贷合同
    Route::get('/api/user/Getjiedaiht', 'Api\UserController@get_jiedai_htinfo');  
    
    Route::any('/api/index/CallBack', 'Api\IndexController@CallBack');
    Route::any('/api/wallet/get_type', 'Api\WalletController@gettype');
    ...
    Route::any('api/getbizcate', 'Api\CurrencyController@bizcate');
    ...
    Route::get("api/US",'Api\DefaultController@US');
    ...
    Route::post('api/user/dologin', 'Api\LoginController@dologin'); //切换模拟账号
    Route::post('api/user/doregister', 'Api\LoginController@doregister');//切换真实账号
    ...
    Route::post('/api/sms_mail', 'Api\SmsController@sendMail');
     Route::post('/api/get_area_code', 'Api\SmsController@get_area_code');
    Route::post('/api/upload', 'Api\DefaultController@upload');
    Route::post('/api/upload_new', 'Api\DefaultController@upload_new');
    ...
    Route::get('api/currency/recharge', 'Api\CurrencyController@listsRecharge');
    Route::get('api/currency/quotation', 'Api\CurrencyController@quotation');
     Route::get('api/currency/optionalquotation', 'Api\CurrencyController@optionalQuotation');  
    //站内信
    ...
    Route::any('api/currency/quotation_huang', 'Api\CurrencyController@huangnewQuotation');
    ...
    Route::any('api/currency/bclass', 'Api\CurrencyController@b_class');  //币种分类
    
    Route::get('api/currency/new_test', 'Api\CurrencyController@test');   //外汇 期货 
    
     Route::get('api/currency/sina_test', 'Api\CurrencyController@sinatest');   //外汇 期货 测试

    Route::any('api/currency/new_timeshar','Api\CurrencyController@klineMarket')->middleware(['cross']);
    Route::any('api/currency/new_tradehistory','Api\CurrencyController@tradeMarket')->middleware(['cross']);
    Route::any('api/currency/kline_market', 'Api\CurrencyController@klineMarket')->middleware(['cross']);
    ...
    Route::group(['prefix' => 'api', 'middleware' => ['check_api', 'XSS', /*'check_user'*/]], function () {
        ...
        Route::get('/user/OrderCenter', 'Api\UserController@OrderList');   //订单中心
          //借贷信息
        Route::get('/user/GetJiedaiList', 'Api\UserController@jiedai_list');   //借贷列表
        Route::get('/user/GetJiedaiInfo', 'Api\UserController@get_jiedai_info');
        Route::post('user/jiedai', 'Api\UserController@savejiedai');  //借贷入库
        Route::post('user/jiedai_return', 'Api\UserController@JiedaiCancle');  //借贷还款
        
        
        
        //AI量化理财
       
       // Route::get('/ai/detail', 'Api\AiController@getDetail');
        Route::post('/ai/buyAi', 'Api\AiController@buyAi');
        Route::post('/ai/cancle', 'Api\AiController@Aicancle');
        
        //量化理财
       
        Route::get('/dual/detail', 'Api\DualController@getDetail');
        Route::post('/dual/buyDual', 'Api\DualController@buyDual');
        Route::post('/dual/dual_cancle', 'Api\DualController@Dualcancle');
        ...
        Route::post('/user/getCreditList', 'Api\UserController@getCreditList');//信用分记录
        ...
        // 新版用户认证信息
        Route::get('real/newstate', 'Api\UserController@newrealState');
        Route::post('real/newreal', 'Api\UserController@newsaveUserReal');
        Route::post('real/updatesPassword', 'Api\UserController@updatesPassword');
        Route::post('real/updatesReceivingAddress', 'Api\UserController@updatesReceivingAddress');
        // 用户认证信息
        Route::get('real/state', 'Api\UserController@realState');
        Route::get('getUSDTTRC20address', 'Api\UserController@getUSDTTRC20address');
        Route::post('user/real', 'Api\UserController@saveUserReal');
        // 用户钱包操作相关
        Route::get('userreal/updatesPassword', 'Api\UserController@userWalletList');
        ...
        Route::post('project/buy', 'Api\CurrencyProjectController@buyOrder');//配售
        ...
        Route::get('/coin/canceltrade', 'Api\CoinTradeController@cancelTrade');
        ...
        Route::post('/currencyapplication/add', 'Api\FeedBackController@currencyAdd');
        ...
        Route::post('wallet/balancelist', 'Api\WalletController@BalanceList');
        Route::post('wallet/Wiretransfer', 'Api\WalletController@Wiretransfer');  //电汇
      ...
      //  Route::get('wallet/earnedhistory', 'Api\WalletController@earnedhistory');
        Route::post('wallet/charge_req', 'Api\WalletController@chargeReq');     //充值
        ...
        Route::post('/lever/submit',
         ...
         Route::post('/lever/dataget',
            ['uses' => 'Api\LeverController@dataget', 'middleware' => ['validate_locked', 'check_user']]);
        ...
        Route::post('/lever/my_ls_order', 'Api\LeverController@mylsOrder');
        ...
        Route::get('follow/traderDetail', 'Api\FollowController@traderDetail'); //交易员详情
         Route::get('follow/Detail', 'Api\FollowController@gdDetail'); //交易员详情
        ...
        Route::prefix('microtrade')->namespace('Api')->group(function () {
            ...
            Route::post('order', 'MicroOrderController@getOrder')->middleware('validate_locked','check_user'); //提交下单
    ...
    Route::group(['prefix' => 'admin', 'middleware' => ['admin_auth']], function () {
        ...
        Route::post('/dual/del', 'Admin\DualController@del');
        
           //AI
        Route::get('/ai/order', 'Admin\AiController@order');
        Route::get('/ai/editAiView', 'Admin\AiController@editAiView');
        Route::get('/ai/order_lists', 'Admin\AiController@order_lists');
        Route::get('/ai/index', 'Admin\AiController@index');
        Route::get('/ai/add', 'Admin\AiController@add');
        Route::get('ai_list', 'Admin\AiController@lists');
        Route::get('/ai/getnewprice', 'Admin\AiController@getNewPrice');
        Route::post('/ai/saveAi', 'Admin\AiController@saveAi');
        Route::post('/ai/editAi', 'Admin\AiController@editAi');
        Route::post('/ai/del', 'Admin\AiController@del');
        ...
        //借贷
        Route::get('/lending/order', 'Admin\LendingController@order');
        Route::get('/lending/editLendingView', 'Admin\LendingController@editLendingView');
        Route::get('/lending/order_lists', 'Admin\LendingController@order_lists');

        Route::get('/lending/index', 'Admin\LendingController@index');
        Route::get('/lending/add', 'Admin\LendingController@add');
        
        Route::post('/lending/del', 'Admin\LendingController@del');
        Route::get('lending_list', 'Admin\LendingController@lists');
        
         Route::get('/lending/getnewprice', 'Admin\LendingController@getNewPrice');
        Route::post('/lending/saveLending', 'Admin\LendingController@saveLending');
        Route::post('/lending/editLending', 'Admin\LendingController@editLending');
        
         //借贷放款 审核
        Route::get('jiedai_show', 'Admin\LendingController@show')->middleware(['demo_limit']);
        Route::post('jiedai_done', 'Admin\LendingController@done')->middleware(['demo_limit']);
        Route::post('adjust_account', 'Admin\LendingController@cancle')->middleware(['demo_limit']);
        ...
        Route::post('/is_open', 'Admin\CurrencyController@is_open');
        ...
        Route::get('/currency/project/detail', 'Admin\CurrencyProjectController@projectDetail');
         ...
         Route::post('/currency/project/del', 'Admin\CurrencyProjectController@delProject');
        ...
        Route::post('/user/real_error', 'Admin\UserRealController@autherror');
        ...
        Route::get('class', 'Admin\CurrencyController@cclass'); //币种分类
        Route::post('/class_transfer', 'Admin\CurrencyController@class_transfer'); //币种分类隐藏开关
        Route::get('currency_listclass', 'Admin\CurrencyController@listsclass');
