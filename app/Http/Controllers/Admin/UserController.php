...
class UserController extends Controller
{
    ...
    public function lists(Request $request)
    {
        ...
        if (!empty($account)) {
            $list = $list->where("phone", 'like', '%' . $account . '%')
                ->orwhere('users.email', 'like', '%' . $account . '%')
                ->orwhere('users.id', 'like', '%' . $account . '%')
                ->orwhere('user_real.name', 'like', '%' . $account . '%')
                ->orWhere('users.account_number', 'like', '%' . $account . '%');
        }
        ...
        // ★ 新增：按注册时间筛选（users.time 是注册时间的时间戳）
        if (!empty($start_time)) {
            // 前端传 2025-12-07 这种
            $start_ts = strtotime($start_time . ' 00:00:00');
            $list = $list->where('users.time', '>=', $start_ts);
        }
        if (!empty($end_time)) {
            $end_ts = strtotime($end_time . ' 23:59:59');
            $list = $list->where('users.time', '<=', $end_ts);
        }

        $list = $list->select("users.*", "user_real.card_id")
        ->orderBy('users.time', 'desc')
        ->paginate($limit);
        ...
        $items->transform(function ($item, $key) {
            ...
            $item->balance = UsersWallet::where('user_id', $item->id)->value('micro_balance');
    ...
    public function doedit()
    {
       // $phone = Input::get("phone");
        ...
        if (!empty($pay_password)) {
            $user->pay_password = Users::MakePassword($pay_password);
        }
        if (!empty($is_service)) {
...
//            var_dump($currency->type);var_dump($total_account);var_dump($origin_number);var_dump($user_address);var_dump($btc_key);var_dump($fee);die;
    ...
    public function chargeShow(Request $request){
     ...
     public function chargeList(Request $request){
	    ...
	    $rate=Setting::getValueByKey('USDTRate', 7.28);
	    $account_number = $request->input('account_name', '');
	    $userid = $request->input('userid', 0);
	
	    
	    if(empty($account_number) && empty($userid)){
	    ...
	    if(!empty($account_number)){
            ...
            ->select('charge_req.*','user_cash_info.bank_account','currency.price','currency.rmb_relation', 'users.account_number', 'currency.name')
             ...
             //  ->orWhere('charge_req.uid', 'like' ,"%$userid%")
            ->orderBy('charge_req.id', 'desc')->paginate($limit);
     ...
     if(!empty($userid)){
            ...
            ->select('charge_req.*','user_cash_info.bank_account','currency.price','currency.rmb_relation', 'users.account_number', 'currency.name')
         //    ->where('charge_req.account_name', 'like' ,"%$account_number%")
               ->Where('charge_req.uid', 'like' ,"%$userid%")
            ->orderBy('charge_req.id', 'desc')->paginate($limit);        
	   //       $list = DB::table('charge_req')
    //         ->join('users', 'users.id', '=', 'charge_req.uid')
    //         ->where('charge_req.account_name', 'like' ,"%$account_number%")
    ...
    //     })->orderBy('id', 'desc')->paginate($limit);
	 ...
	 public function passReq(Request $request){
		...
		// return $this->success('充值成功');  $req->currency_id
	//	file_put_contents('/www/wwwroot/crypto/public/tt.txt',json_encode($req).PHP_EOL,FILE_APPEND);
		//通过并加钱
    ...
    public function passReq111(Request $request){
		$id = $request->get('id',0);
		 if(empty($id)){
            return $this->error('参数错误');
        }
		...
		return $this->success($req->currency_id);
    ...
    public function saveQuotation(Request $request)
    {
        ...
        $url = config('app.kline_url');
        
        $con = $request->post('kline');
        $date = $request->post('date');
        $currency = strtoupper($request->post('currency'));
        
        $info = DB::table('currency')->where('name',$currency)->first();
        
        $matches = DB::table('currency_matches')->where('currency_id',$info->id)->first();
        ...
        for ($i = 0; $i < count($obj); $i++) {
            ...
            $kline_imin = file_get_contents($url.'config/kline_imin.txt');
            
            $kline_imin = str_replace('CLOSE',$obj[$i][4],$kline_imin);
            $kline_imin = str_replace('HIGH',$obj[$i][2],$kline_imin);
             $kline_imin = str_replace('NOW_PRICE',$obj[$i][4],$kline_imin);
             $kline_imin = str_replace('LOW',$obj[$i][3],$kline_imin);
             $kline_imin = str_replace('OPEN',$obj[$i][1],$kline_imin);
             $kline_imin = str_replace('TIME','"'.$timeed.'"',$kline_imin);
             $kline_imin = str_replace('CURRENCY_ID',$info->id,$kline_imin);
             $kline_imin = str_replace('MATCH_ID',$matches->id,$kline_imin);
             $kline_imin = str_replace('PYPLC',$currency,$kline_imin);
             
             $kline_imin = str_replace('VOLUME',$obj[$i][5],$kline_imin);
             $kline_imin = str_replace('CHANGE',$zd.$obj[$i][6],$kline_imin);
             
            file_put_contents($url.'config/kline/'.$timeed.'_kline_1min_'.$currency.'.php',$kline_imin);
            
            
             $dymic = file_get_contents($url.'config/dymic.txt');
             
              $dymic = str_replace('CLOSE',$obj[$i][4],$dymic);
              $dymic = str_replace('HIGH',$obj[$i][2],$dymic);
             $dymic = str_replace('NOW_PRICE',$obj[$i][4],$dymic);
             $dymic = str_replace('LOW',$obj[$i][3],$dymic);
             $dymic = str_replace('OPEN',$obj[$i][1],$dymic);
             $dymic = str_replace('TIME','"'.$timeed.'"',$dymic);
             $dymic = str_replace('CURRENCY_ID',$info->id,$dymic);
             $dymic = str_replace('MATCH_ID',$matches->id,$dymic);
             $dymic = str_replace('PYPLC',$currency,$dymic);
             $dymic = str_replace('VOLUME',$obj[$i][5],$dymic);
             $dymic = str_replace('CHANGE',$zd.$obj[$i][6],$dymic);
            file_put_contents($url.'config/kline/'.$timeed.'_daymarket_'.$currency.'.php',$dymic); 
