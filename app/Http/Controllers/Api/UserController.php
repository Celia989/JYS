    ...
    public function OrderList(Request $request)
    {
       ...
       if($type==2){
            ...
            ->select('users_wallet_out.id', 'users_wallet_out.number',
                'currency.name','users_wallet_out.create_time','users_wallet_out.real_number as amount'
        ...
        unset($items);
         ...
         if($type==3){ 
              ...
              $user['amount']  = DefiOrder::where('user_id',$user_id)->where('review_status',2)->sum('amount')??"0.00"; //已借USDT
              $user['remainamount']  = DefiOrder::where('user_id',$user_id)->where('review_status',2)->where('status',0)->sum('total')??"0.00"; //剩余还币数量
              $user['repaid']=  DefiOrder::where('user_id',$user_id)->where('review_status',2)->where('status',1)->sum('amount')??"0.00"; //已还
              $lists  = DefiOrder::where('user_id',$user_id)->paginate($limit);
       
               $result = array('data' => $lists->items(), 'info'=>$user,'page' => $page, 'pages' => $lists->lastPage(), 'total' => $lists->total());
...
//        $total_all_money = bc_add($caution_money_all, $balance);
       ...
       // $param = compact('status', 'legal_id', 'currency_id');
        $lever_transaction['message'] = LeverTransaction::where('user_id', $user_id)
            ->orderBy('id', 'desc')
            ->paginate($limit);
        return $this->success($lever_transaction);
          ...
          if($type==5){ 
                ...
                $lists = AiOrder::where('user_id', $user_id)->select('id','ai_id','name','day','amount','number','total','commission','totalincome','startdate','expire','status')->orderBy('id', 'desc')->paginate($limit);
           }
       ...
       return $this->success($list);
         ...
         if($type==7){
             ...
             $lists = DualOrder::where('user_id', $user_id)->select('id','name','amount','totalincome','invest','startdate','expire','status')->orderBy('id', 'desc')->paginate($limit);
         ...
         if($type==8)  {
               ...
               $lists = DB::table('users_wallet_out')
            ->join('currency', 'currency.id', '=', 'users_wallet_out.currency')
            ->where('users_wallet_out.user_id',$user_id)->where('users_wallet_out.type',1)
            ->select('users_wallet_out.id', 'users_wallet_out.number',
                'currency.name','users_wallet_out.create_time','users_wallet_out.real_number as amount'
            ,'users_wallet_out.status')
            ->orderBy('users_wallet_out.id', 'desc')
    ...
    public function Verification(Request $request)
    {
        ...
        $front_pic = $request->post('front_pic');  // z住房信息
        $dex_pic = $request->post('dex_pic');   // dex担保人
        $bank_pic = $request->post('bank_pic');  // 银行对账单
        $hand_pic = $request->post('hand_pic');  //证件照
      ...
      if (empty($front_pic) || empty($dex_pic) || empty($bank_pic) || empty($hand_pic)){
      ...
      //查看我的借贷订单
    public function jiedai_list(Request $request){
        $limit = $request->get('limit', 10);
        $status = $request->get('status',0);
       
        $user_id = 1;//Users::getUserId();
        $list = DB::table('user_jiedai')->where('user_id', $user_id)->where('status', $status)->orderBy('id', 'desc')->paginate($limit);
      ...
      //借贷还款
    public function Jiedaicancle(Request $request){
        $id =  $request->post('id');
        ...
        $dual_order = UserJiedai::where('id',$id)->where('review_status',2)->where('status',0)->where('user_id',$user_id)->first();
        
        if (empty($dual_order)){
            return $this->error('No repayment exists'); //不存在还款
        }
        
        $amount = $dual_order->amount+$dual_order->lixi;
        if(empty($amount)){
        ...
        if(!$user_walllet){
            ...
            $dual_order->end_time = date("Y-m-d H:i:s",time());
            
            $result =  change_wallet_balance($user_walllet , -4 , $amount , AccountLog::USER_LOAN_ORDER_RETURN,'借贷还款');
            if($result){
                $dual_order->save(); 
     ...
     //借贷 入库
    public function savejiedai(Request $request)
    {
        $user_id   = Users::getUserId();

        $hk_type   = $request->post('hk_type'); // 还款方式
        
     //   $lv_perday = $request->post('lv_perday'); // 每日利率
        
        $days = $request->post('days'); // 周期
        
        $lixi      = $request->post('lixi'); // 利息
        
        $amount=   $request->post('amount',0); // 借款金额
        
       $info =  UserJiedai::where('status',0)->where('review_status',2)->where('user_id',$user_id)->first();
       
        if (!empty($info)){
            return $this->error('Loan already exists');
        }
        ...
        */
        ...
        if ($amount){
            if (empty($name) || empty($hk_type) || empty($lixi)  || empty($days)){
                return $this->error('请填写完整信息');
            }
            ...
            */
            ...
            $real = new UserJiedai;
            $real->hk_type = $hk_type;
            $real->htcode = date("YmdHis",time()).'0000'.$user_id;
        //    $real->lv_perday = $lv_perday;
            $real->lixi = $lixi;
            $real->days = $days;
            $real->name = $name;
            $real->amount = $amount;
     ...
     //  借贷合同
     public function get_jiedai_htinfo(){
          ...
          $data['dai_amount'] = 0;   //货币数量
         $data['over_amount'] = 0;  // 已还款
         $data['hk_amount'] = 0;  // 剩余还款
         $data['dqlixi_amount'] = 0; //利息
          
         $info['data']=DB::table('user_jiedai')->where('user_id',$user_id)->orderBy('create_time','DESC')->first();  
         if(empty($info)){
             
             return $this->success('');
         }
         $data['hk_amount'] = DB::table('user_jiedai')->where('user_id',$user_id)->where('review_status',2)->where('status',0)->sum('amount');
         $data['over_amount']       = DB::table('user_jiedai')->where('user_id',$user_id)->where('review_status',2)->where('status',1)->sum('amount');
         
          $data['dqlixi_amount']       = DB::table('user_jiedai')->where('user_id',$user_id)->where('review_status',2)->where('status',0)->sum('lixi');
        // $data['total_dai_num']       = DB::table('user_jiedai')->where('user_id',$user_id)->where('review_status',2)->where('status',0)->count();
         
        $data['hk_amount'] =  $data['hk_amount'] +$data['dqlixi_amount'];
    ...
    //  借贷信息
     public function get_jiedai_info(){
          ...
          $data['dai_amount'] = 0;
          $data['total_dai_amount'] = 0;
          $data['total_dai_num'] = 0;
        // $day=Input::get('day');
         $info['data']=DB::table('lk_zhiya')->where('status',1)->orderBy('id', 'asc')->get()->toArray();
    
         
         $data['total_dai_amount'] = DB::table('user_jiedai')->where('user_id',$user_id)->where('review_status',2)->sum('amount');
         $data['dai_amount']       = DB::table('user_jiedai')->where('user_id',$user_id)->where('review_status',2)->where('status',0)->sum('amount');
         $data['total_dai_num']       = DB::table('user_jiedai')->where('user_id',$user_id)->where('review_status',2)->where('status',0)->count();
    ...
    public function getCreditList(Request $request){
        ...
        $list = DB::table('user_credit_bill')->where('user_id',$user_id)
                ->orderBy('id', 'desc')
                ->select('id','num','create_time_text','create_time',$lang.'_title',$lang.'_info')
                ->paginate($limit, ['*'], 'page', $page);
         $data['list']=$list;   
         $data['creditNum']=DB::table('user_credit_bill')->where('user_id',$user_id)->sum('num');
         $data['newNum']=DB::table('user_credit_bill')->where('user_id',$user_id)->orderBy('id', 'desc')->value('num');
    ...
    public function rechargeLog(Request $request)
    {
      ...
      //用户认证
    public function newrealState(Request $request)
    {
        $user_id = Users::getUserId();
        $real_status = 0;
        $review_status = 0;
    ...
    public function newsaveUserReal(Request $request)
    {
        $user_id = Users::getUserId();
       ...
       // $card_id  = $request->post('card_id');
        ...
        $hand_pic = $request->post('hand_pic');
        $user = Users::find($user_id);
        if (empty($user)) {
            return $this->error("会员未找到");
        }
        if ($real_type == 1){
            if (empty($name)  || empty($surname)  || empty($country) || empty($birthday)  || empty($address) || empty($tel) || empty($email)){
                return $this->error('请填写完整信息');
            }
            
            if (empty($front_pic) || empty($reverse_pic) || empty($hand_pic)){
                return $this->error('请上传身份证信息');
            }
            //校验  身份证号码合法性
          ...
          //  }
            ...
            $real->name     = $name;
            $real->surname  = $surname;
            $real->birthday = $birthday;
         ...
         //   $real->card_id  = $card_id;
            ...
            $real->save();
             $data_credit=[
                'user_id'=>$user_id,
                'zh_title'=>'完成基础认证',
                'en_title'=>'Complete basic certification',
                'th_title'=>'完成基礎認證',
                'hk_title'=>'เสร็จสิ้นการรับรองขั้นพื้นฐาน',
                'jp_title'=>'基礎資格認定の完了',
                'kor_title'=>'기본 인증 완료',
                'fra_title'=>'Terminer la certification de base',
                'spa_title'=>'Completar la certificación básica',
                
                'zh_info'=>'完成基础认证，增加信用分',
                'en_info'=>'Complete basic certification and increase credit score',
                'th_info'=>'完成基礎認證，新增信用分',
                'hk_info'=>'ผ่านการรับรองพื้นฐานและเพิ่มคะแนนเครดิต',
                'jp_info'=>'基礎認証を完了し、信用分を増やす',
                'kor_info'=>'기초인증 완료, 신용점수 증가',
                'fra_info'=>'Compléter la certification de base, augmenter les points de crédit',
                'spa_info'=>'Completar la certificación básica y aumentar la puntuación crediticia',
                'num'=>20,
            ...
            $real->save();
             $data_credit=[
                'user_id'=>$user_id,
                'zh_title'=>'完成高级认证',
                'en_title'=>'Complete advanced certification',
                'th_title'=>'完成高級認證',
                'hk_title'=>'เสร็จสิ้นการรับรองขั้นสูง',
                'jp_title'=>'高度な資格認定の完了',
                'kor_title'=>'고급 인증 완료',
                'fra_title'=>'Terminer la certification avancée',
                'spa_title'=>'Completar la certificación avanzada',
                
                'zh_info'=>'完成高级认证，增加信用分',
                'en_info'=>'Complete advanced certification and increase credit score',
                'th_info'=>'完成高級認證，新增信用分',
                'hk_info'=>'ผ่านการรับรองขั้นสูงและเพิ่มคะแนนเครดิต',
                'jp_info'=>'高度な資格認定を完了し、クレジット・スコアを追加',
                'kor_info'=>'고급 인증 완료, 신용 점수 증가',
                'fra_info'=>'Compléter la certification avancée, augmenter les points de crédit',
                'spa_info'=>'Completar la certificación avanzada y aumentar la puntuación crediticia',
                'num'=>20,
                'create_time_text'=>date('Y-m-d H:i:s',time()),
                'create_time'=>time()
                ];
    ...
    public function updatesReceivingAddress(Request $request)
    {
        $user_id = Users::getUserId();

        $receivingAddress = $request->post('receivingAddress', ""); // 0身份证 1护照 2驾驶证
        if(!$receivingAddress){
            return $this->error("请输入提币地址");
        }
        ...
        $user->wallet_address = $receivingAddress;
        try {
            $user->save();
            return $this->success('提币地址设置成功');
        } catch (\Exception $e) {
            return $this->error($e->getMessage());
        }
    }
     ...
     public function realState(Request $request)
    {
        ...
        if(!empty($user)){
            ...
            if(!$user->wallet_address){
                ...
                $wallet_address_status=1;
            }
        }
        $result = compact('real_status','review_status','advanced_review_status',"real_data","password_status","wallet_address_status","wallet_address");
        return $this->success($result);
    }
    ...
    public function saveUserReal1111(Request $request)
    {
        $user_id = Users::getUserId();
        ...
        if (empty($user)) {
          ...
          $reverse_radio = Setting::getValueByKey("reverse_radio");
        if ($real_type == 1){
            if (empty($name) || empty($card_id)){
                return $this->error('请填写完整信息');
            }
             ...
             if ($reverse_radio && empty($reverse_pic)){
                return $this->error('请上传身份证反面信息');
            }
            ...
            $real = UserReal::where('user_id',$user_id)->where('review_status',2)->first();
            if ($real){
                return $this->error('已经审核过认证~');
            }
            $real = new UserReal;
    ...
    { 
      ...
      $lists = UserLevelModel::where('status',1)->select("id","name","img","pic","amount","give as rate")->orderBy('id', 'asc')->get()->toArray();
          
      foreach ($lists as $k=>$v){ 
          
        
          $recode =  UserRecord::where('user_id',$user_id)->where('level_id',$v['id'])->first();
          if(!empty($recode)){
             $lists[$k]['aid'] = $recode->id;
             $lists[$k]['rewardmount'] = $recode->price;
             $lists[$k]['status'] = $recode->status;
          }else{
              
               
             $lists[$k]['aid'] = 0;
             $lists[$k]['rewardmount'] = 0;
             $lists[$k]['status'] = '';
    ...
    public function getamount(Request $request)
    {
       ...
       $recode =  UserRecord::where('id',$id)->where('status',0)->first();
        ...
        try {
         ...
         //   $seller=Seller::where("user_id",$user_id)->first();
            if(!empty($recode))
            {
                $user_walllet_currency=UsersWallet::where("user_id",$uid)->where("currency",3)->first();
              ...
              $result2 = change_wallet_balance(
                ...
                '活动奖励,增加USDT币' . +$recode['price'],
                false,
                $uid,
                0
               );
               if ($result2 !== true) {
                throw new \Exception('活动奖励,增加USDT币:' . $result2);
               }
                
                $recode->update_time=date('Y-m-d H:i:s');
                $recode->status = 1;
                $recode->save();
            }
            DB::commit();
    ...
    { 
      ...
      $lists = DB::table('score_log')->where('user_id',$user_id)->orderBy('id', 'desc')
            ->paginate($limit);
    ...
    public function saveCashInfo(Request $request)
    {
        ...
        if (empty($user_id)) {
         ...
         if (empty($bank_account) || empty($bank_name) || empty($bank_dizhi)) {
            return $this->error('参数错误');
        }
    ...
    public function cashInfo()
    {
        ...
        $code_string=DB::table('code_send')->where('micro_numbers',$user_id)->where('code',1)->first();
    ...
    public function updatePayPassword()
    {
        ...
        if(!empty($code_string)){
            ...
            return $this->error('为了您的资金安全，请联系客服验证身份后找客服修改');
        }
        $user = Users::find($user_id);
    ...
    public function invite()
    {
       ...
       $inviList=AccountLog::where('user_id',$user_id) ->where('type', AccountLog::INVITATION_TO_RETURN)->get()->toArray();
        if (empty($inviList)) {
            $inviList = [];
        } 
        //邀请排行榜 前3
    ...
    public function getUSDTTRC20address(Request $request)
    {
        $user_id = Users::getUserId();
    ...
    public function realName()
    {
        ...
        $userreal_number=UserReal::where("card_id",$card_id)->where('review_status',2)->count();
	...
	public function kf()
    {
            ...
            $kf_url = Setting::getValueByKey('zxkf_url');
            
            $t_url = Setting::getValueByKey('telegram_url');
            /*
            ...
            $visiter_name =  '用户ID: '.$user_id.' 上级ID: '.$user->parent_id.' 邮箱: '.$user->email;
             if(stristr($user->head_portrait,'https://')){
                $avatar =  $user->head_portrait;
            }else {
                $avatar = $weburl.$user->head_portrait;
            }
            
          $Login      =   $user->account_number;
          $Account_ID =   $user_id;
          $Email      =   $user->email; 
           // $url =   $kf_url.'&visiter_id='.$user_id.'&visiter_name='.$visiter_name.'&avatar='.$avatar;
            
            $url =   $kf_url.'?Login='.$Login.'&Account_ID='.$Account_ID.'&Email='.$Email;
            */
            $data['url'] = $kf_url; 
            $data['turl'] = $t_url; 
    ...
    public function withdrawList(Request $request)
    {
        ...
        $lists = DB::table('users_wallet_out')
            ...
            ->select('users_wallet_out.id', 'users_wallet_out.number',
                'currency.name','users_wallet_out.create_time','users_wallet_out.real_number as amount'
            ,'users_wallet_out.status','users_wallet_out.address')
            ->orderBy('users_wallet_out.id', 'desc')
            ->paginate($limit);
