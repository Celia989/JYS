...
//        var_dump($rsp);
    ...
    public function sina_s($name,$period){
        ...
        // ★★ 用最新一根 K 线的 close 同步行情价格 ★★
        try {
            if (!empty($result)) {
                $last = end($result);              // 最后一根 K 线
                if (isset($last['close'])) {
                    $close = $last['close'];

                    // 用 $name 找到币种和交易对（例如 BTC -> BTC/USDT）
                    $currency = Currency::where('name', $name)->first();
                    if ($currency) {
                        $match = CurrencyMatch::where('currency_id', $currency->id)
                            ->where('is_display', 1)
                            ->first();

                        if ($match) {
                            // 1) 更新交易对表里的 now_price（列表直接用的就是它）
                            $match->now_price = $close;
                            $match->save();

                            // 2) 更新 currency_quotation 表（自选、涨幅榜等用到）
                            CurrencyQuotation::updateTodayPriceTable([
                                'legal_id'    => $match->legal_id,
                                'currency_id' => $match->currency_id,
                                'now_price'   => $close,
                                // 不传 volume，保留原值；change 会在方法内部自动计算
                            ]);
                        }
                    }
                }
            ...
            \Log::error('sync now_price from sina_s error: '.$e->getMessage());
        }
    ...
    public function foreign($name,$period){
        ...
        // 同步 CurrencyMatch + CurrencyQuotation，代码跟 sina_s 里的一模一样
        // ...（复制上面的 try 部分）...
    ...
    public function newTimeshars(Request $request)
    {
        ...
        if ($fFVCeHJ->count() > 0) {
            foreach ($fFVCeHJ as $qhNdsMJ => $ncfMSiJ) {
                $KuIMXuv = array(
                    'open'   => $ncfMSiJ->start_price,
                    'close'  => $ncfMSiJ->end_price,
                    'high'   => $ncfMSiJ->highest,
                    'low'    => $ncfMSiJ->mminimum,
                    'volume' => $ncfMSiJ->number,
                    'time'   => $ncfMSiJ->day_time * 1000,
                );
                $rXDaOwJ[] = $KuIMXuv;
            }

            // ✅ 用最后一根 K 线的 close 更新 currency_quotation.now_price
            $lastKline = end($rXDaOwJ);
            if ($lastKline && isset($lastKline['close'])) {

                $lastClose = $lastKline['close'];

                // 先找到 BTC/USDT 对应的那条 currency_matches
                $match = \App\CurrencyMatch::where('currency_id', $ElYLALJ)   // BTC 的 id
                ->where('legal_id', $jqbYHhJ)                              // USDT 的 id
                ->first();

                if ($match) {
                    // 用 match_id 来更新 currency_quotation 这条记录
                    DB::table('currency_quotation')
                        ->where('match_id', $match->id)
                        ->update([
                            'now_price' => $lastClose,
                        ]);

                    // （可选）如果你想一起把 close 也改掉：
                    // DB::table('currency_quotation')
                    //     ->where('match_id', $match->id)
                    //     ->update([
                    //         'now_price' => $lastClose,
...
//    }
    ...
    public function huangnewQuotation()
    {
        // 前端传过来的分类 id（外汇 / 美股 / 期货 / 数字货币 ...）
        $is_class = Input::get('is_class', 0);

        // 1. 先找出属于这个分类的币种 id
        //    currency.is_class = 分类id
        $currencyIds = Currency::where('is_class', $is_class)
            ->where('is_display', 1)
            ->pluck('id')
            ->toArray();

        // 如果这个分类下面没有任何币，直接返回空数组
        if (empty($currencyIds)) {
            return $this->success([]);
        }

        // 2. 按 newQuotation 的结构，取出法币 (USDT 等) + 其下的行情列表
        //    只是把 quotation 里只保留 currency_id 属于这个分类的
        $list = Currency::with(['quotation' => function ($query) use ($currencyIds) {
            $query->where('is_display', 1)
                ->where('id', '<>', 4)
                ->whereIn('currency_id', $currencyIds)    // ✅ 只要属于该分类的币
                ->orderBy('sort', 'asc');
        }])
            ->whereHas('quotation', function ($query) use ($currencyIds) {
                $query->where('is_display', 1)
